...
## 목차
1. [캐릭터 컨셉](#1-캐릭터-컨셉)
2. [스탠딩 일러스트 제작 과정](#2-스탠딩-일러스트-제작-과정)
3. [타이틀 일러스트 제작 과정](#3-타이틀-일러스트-제작-과정)
4. [작업 후기](#4-작업-후기)
...
* [[OpenPose] Standing](https://civitai.com/models/128029/openpose-standing) 에서 구한 OpenPose 이미지를 ControlNet 에 사용, 생성되는 캐릭터 이미지의 체형을 제어할 수 있었음

## 문제 2: 전체가 마음에 드는 이미지가 나오지 않음

![](images/CharacterTries.png)  
* 위와 같이 이미지 생성을 반복해봤지만 전체가 마음에 드는 이미지가 나오지 않았음
  * 주황색 머리카락을 의도했지만 orange hair, brown hair 를 지정해도 원하는 색깔이 나오지 않음
  * 어떤 이미지든 모양이나 색이 원하지 않는 형태인 부분이 포함되어 있음
...
* 생성된 이미지 중 의도에 가까운 부분을 가진 이미지들을 모음
...
* XD 표정은 해당 표정 만들어주는 Lora 사용([Comic expression eyes 1](https://civitai.com/models/134236))
  * 메이플스토리 Lora 비중을 낮춰야 표정 Lora 가 제대로 적용됐음
...
* OpenPose 로 캐릭터 구도를 잡음
  * [[OpenPose] Head rest](https://civitai.com/models/141049)

![](images/00020-830103208.png){: width="600"}  
* 밑그림으로 사용하기에 적당한 배경 구도가 나올 때 까지 이미지 생성을 반복
...
* 밑그림으로부터 mlsd 추출, OpenPose 와 조합하여 이미지 생성함(1시간 소요)
...
# 4. 작업 후기
## 반복 작업의 효율화 필요
* 원하는 이미지, 또는 영감을 주는 이미지가 나올 때 까지의 반복을 얼마나 효율적으로 할 수 있는지가 중요하다는 생각이 듦
...
* 저화질 이미지로 구도를 먼저 파악하고 Step 수와 크기를 늘려가며 작업했지만, 저화질 이미지에서 마음에 들었던 부분과 차이가 발생
...
* 물론 학습 재료가 될 캐릭터 이미지를 직접 만들어 낼 수 있었다면 AI를 안썼겠지만...
* (1)범위를 좁힌 (2)대량의 이미지를 (3)빠르게 뽑아내 (4)선별할 수 있는 시스템을 구축할 수 있다면 베스트일 듯

### 사람의 숙련도 높이기
* AI가 100% 완전한 그림을 만들어 줄 수도 있겠지만 그러기에는 너무나 많은 시간과 비용 들어감
  * 특히 게임의 주요 리소스로 사용한다면 더욱 완성도에 신경써야 함
...
* 소규모 게임이면 어느정도 넘어갈 수 있겠지만, 어설픈 이미지는 대작 게임에서 그대로 쓰기에는 확실히 무리가 있음
* 각 프로젝트의 상황이나 AI 이미지를 사용하는 목적에 따라 잘 판단해야 될 듯 함
  * 게임의 주요 리소스가 아니라면 이렇게까지 신경 쓰지는 않아도 될지도

### 작품성
* 타이틀 일러스트의 경우, "캐릭터가 책상에 앉아있다"는 정보는 전달되지만 재미가 부족함
* 감정을 불러일으킬 수 있는 그림을 만들기 위해서는 AI를 사용하더라도 역시 사람의 미술적 능력이 필요함
  * 포즈, 구도, 광원, 색감, 상황 등을 설정

## 결론 - 결국은 사람이다

| --- | --- |
| ![](images/00016-4075200446.png){: width="300"} | ![](images/00018-1045695746.png){: width="600"} |
...
* 원하는 느낌을 의도적으로 뽑아낼 수 있도록 설계하는 사람의 능력이 중요해 보임